...
export async function main(ns) {
  ...
  while (true) {
    const targetServersAvailableToHack = getBestAvailableTargetsToHackDESC(ns).map(({ server }) => server);
    targetServersAvailableToHack.forEach(server => getAdminRights(ns, server.hostname));
    const targetServersInfo = getServersHackingInfo(ns, targetServersAvailableToHack);
